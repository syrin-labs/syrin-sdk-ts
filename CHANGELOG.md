...
- No `adapters: [...]` argument needed. The SDK installs instrumentation automatically.
...
- **Tier 2** (framework adapters) sets `FrameworkContext` via `AsyncLocalStorage` so
  Tier 1 can enrich `LLM_CALL` events with framework metadata (chain ID, graph node,
  agent name, etc.).
...
- `ConfigStore` — namespaced, versioned, validated config sections (`llm`, `langgraph`,
  `mastra`, `vercel_ai`, `crewai`, `agno`, `ag2`, plus `agents.<id>.*` per-agent namespaces).
- Priority stack (highest → lowest): governance override → anchors → local → remote → defaults.
- `ConfigGuard` — wraps a config apply with health-check and auto-revert on failure.
...
- `AutoRevert` — rolls back config automatically if a health check fails within a TTL.
...
- `modelConfigHash` — hash of temperature, max_tokens, model, and similar fields.
...
o1, o1-mini, o3, o3-mini, Claude 3.5 Sonnet, Claude 3.5 Haiku, and Claude 3 Opus.
...
- Requires Node.js ≥ 18.
- Zero required runtime dependencies beyond `opentelemetry-sdk` packages.
- All adapter imports are lazy — if a library is not installed, its adapter skips
  installation silently.
- Fail-open guarantee: SDK crash never blocks the underlying LLM call.
- Published as ESM with `exports` map and CJS compatibility entry points.
